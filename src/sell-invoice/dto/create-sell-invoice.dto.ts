import { IsNotEmpty, IsNumber, ValidateNested, IsArray, IsString, Validate, IsBoolean, IsOptional, ArrayNotEmpty, ArrayUnique, IsEmail, IsEnum, Matches, Max } from 'class-validator';
...
export class CreateSellInvoiceDto {
    ...
    @IsNotEmpty()
    @Validate(IsDateTime, { message: 'Sell date must be in the format yyyy-mm-dd hh:ii:ss' })
    @Validate(IsBeforeOrToday, { message: 'Sell date should be today or a past date' })
    sellDate: string;
    ...
    @ArrayUnique((product: SellInvoiceProductDto) => product.productId, { message: 'Products array should contain unique products' })
