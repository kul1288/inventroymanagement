...
export class PurchaseInvoiceService {
    ...
    async create(createPurchaseInvoiceDto: CreatePurchaseInvoiceDto): Promise<PurchaseInvoice> {
        const { vendorId, purchaseDate, tax, type, products } = createPurchaseInvoiceDto;
    ...
    async findAll(page: number = 1, limit: number = 10, startDate?: Date, endDate?: Date): Promise<{ data: PurchaseInvoice[], count: number }> {
        const query = this.purchaseInvoiceRepository.createQueryBuilder('purchaseInvoice')
        ...
        if (startDate) {
            query.andWhere('purchaseInvoice.purchaseDate >= :startDate', { startDate });
        }
    ...
    async delete(id: number): Promise<void> {
        return await this.dataSource.transaction(async manager => {
            const purchaseInvoice = await manager.findOne(PurchaseInvoice, { where: { id }, relations: ['products', 'products.product'] });
            if (!purchaseInvoice) {
            ...
            for (const product of purchaseInvoice.products) {
                if (product.remainingQuantity !== product.quantity) {
                    throw new BadRequestException('Cannot delete purchase invoice with products that have been partially used');
                }
            }

            for (const product of purchaseInvoice.products) {
                const productEntity = await manager.findOne(Product, { where: { id: product.product.id } });
                if (productEntity) {
